...
Реализовать на языке Go полноценный распределенный MapReduce сложно, в том числе
из-за того, что Go компилируется в нативный машинный код. В то время как Java
комплируется в универсальный JAR с байткодом, который  можно сабмитить напрямую
в отдельно запущенный кластор с воркерами (вроде бы через команду `hadoop jar`,
если речь идет про Hadoop), вне зависимо от архитектуры цпу клиента/кластера. 
